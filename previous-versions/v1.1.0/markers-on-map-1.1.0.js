    ...
    markerObjects: [ // map markers options => at least one marker object required
        {
            ...
            markerCallback: null, // v1.1.0 and next versions => optional => A callback function can be used when marker element clicked
        }
    ],
}
...
var momNewOptions;
var momMarkersArray = [];
var momGoogleMapsApiResponse;
var momGoogleMapsInitialized;
...
var MarkersOnMap = {
    ...
    Remarker: function (newMarkerObjects) {
        ...
        // check google map initialize to Remarker on
        var googleMapInitializedTimeout = 0;
        var checkGoogleMapInitialized = setInterval(function () {
            googleMapInitializedTimeout++;

            // if waited Google Maps initialization more than 2 minutes on
            if (googleMapInitializedTimeout >= 1200) {

                // clear interval
                clearInterval(checkGoogleMapInitialized);

                // return information
                momConsoleLog('Can not connect to Google Maps services.');
                return false;
            }
            // if waited Google Maps initialization more than 2 minutes off

            // if map initialized on
            if (momGoogleMapsInitialized) {
            ...
            // if map initialized off

        }, 100);
        // check google map initialize to Remarker off
    ...
    Run: function (domMapElement) {
        ...
        // map element css position backup for loading
        var mapElementPositionBackup;

        // if map element not exist on the document => return false
        if (!mapElement) {
            momConsoleError('You called the MarkersOnMap with "' + mapElement + '" selector, but there is no such element on the document.')
            return false;
        }
        // else loading indicator
            ...
            // loading on
            mapElementPositionBackup = window.getComputedStyle(mapElement, null).getPropertyValue('position').toString().toLowerCase();
            if (mapElementPositionBackup !== 'absolute' || mapElementPositionBackup !== 'fixed' || mapElementPositionBackup !== 'sticky') {
        ...
        // wait for google callback to Run on
        var waitForGoogleApiTimeout = 0;
        var waitForGoogleApiResponse = setInterval(function () {
            waitForGoogleApiTimeout++;

            // if waited Google Api more than 2 minutes on
            if (waitForGoogleApiTimeout >= 1200) {

                // clear interval
                clearInterval(waitForGoogleApiResponse);
            ...
            // if waited Google Api more than 20 seconds off

            // if "momGoogleMapsApiResponse" is true on
            if (momGoogleMapsApiResponse) {

                // old position from backup
                mapElement.style.position = mapElementPositionBackup;
            ...
            // if "momGoogleMapsApiResponse" is true off

        }, 100);
        // wait for google callback to Run off
    },
}
...
var momEachMarkerInitAndSet = function (markerObjects) {
    ...
    if (markerObjects && markerObjects.length > 0) {
        ...
        for (var i = 0; i < markerObjects.length; i++) {
            ...
            momSingleMarkerInitAndSet(markersLat, markersLong, markersTitle, markersSize, markersUrl, markersLabel, markersAnimation, markersAdjust, markersContent, markersPlacesApi, markersQuery, markersButtonText, markersCallback);
        }
...
var momSingleMarkerInitAndSet = function (mLat, mLong, mTitle, mSize, mUrl, mLabel, mAnimation, mAdjust, mContent, mPlacesApi, mQuery, mButtonText, mCallback) {
    ...
    function getPlaceDetailsByLatLong(markLat, markLong, markerQuery) {
        ...
        function callback(results, status) {
            if (status == google.maps.places.PlacesServiceStatus.OK) {
                ...
                if (place.photos && (place.photos.length >= 1 ? 1 : place.photos.length) > 0) {
                    // reset single media
                    singleMedia = '';
